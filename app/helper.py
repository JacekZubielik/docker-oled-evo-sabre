...
def read_config():
	config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'oled4docker.cfg'))
...
def get_player_mac(default_gateway_interface):
	if len(process_params("MAC")) > 1 :
		...
		if mac != "":
			...
			logger.warning("MAC Discovery failed. Using 02:42:c0:a8:5a:14")
			return "02:42:c0:a8:5a:14"
